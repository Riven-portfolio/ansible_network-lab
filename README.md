...
將企業級網路配置轉化為可重現的自動化部署流程，展現 Network as Code (NaC) 實踐與基礎設施工程能力。

## 💡 專案背景

### 傳統網路配置面臨的挑戰
- ❌ **人為錯誤**：手動配置 10 台設備需要 2-3 小時，容易出錯
- ❌ **配置不一致**：設備間的配置差異難以發現和修正
- ❌ **變更難追溯**：無法確認「誰在何時改了什麼」
- ❌ **知識難傳承**：新人需要大量時間理解複雜的網路架構

### Network as Code 解決方案
本專案使用 **Ansible** 實現基礎設施即代碼（IaC），將複雜的企業網路拓撲程式化管理：
- ✅ **自動化部署**：15 分鐘完成完整環境配置
- ✅ **可重現性**：100% 一致的配置，消除人為錯誤
- ✅ **版本控制**：透過 Git 追溯所有變更記錄
- ✅ **模組化設計**：階段式部署，降低複雜度
